...
def encrypt_file_data(file_data: bytes, password: str) -> bytes:
    ...
    if not password:
        raise ValueError("Password diperlukan.")
    if len(file_data) == 0:
        raise ValueError("File kosong.")
    if len(file_data) > 16 * 1024 * 1024:
        raise ValueError("Ukuran file maksimal 16 MB.")
    ...
    # CBC mode encryption with key-derived keystream
    encrypted_blocks = []
    prev = iv
    for i in range(0, len(padded), 16):
        block = padded[i:i+16]
        xored = _xor_bytes(block, prev)
        h = hashlib.sha256(key + struct.pack('>I', i // 16) + iv).digest()[:16]
...
def decrypt_file_data(encrypted_data: bytes, password: str) -> bytes:
    ...
    if not encrypted_data.startswith(magic):
        raise ValueError("Format file tidak valid. Bukan file terenkripsi CipherLab.")
    ...
    if len(cipher_data) % 16 != 0:
        raise ValueError("Data terenkripsi rusak.")

    key = derive_key(password, salt)

    # CBC mode decryption with key-derived keystream
    decrypted_blocks = []
    prev = iv
    ...
    try:
        ...
        raise ValueError("Password salah atau file rusak.")
